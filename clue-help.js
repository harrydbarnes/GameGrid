(() => {
  ...
  function currentPuzzle(){const mode=document.querySelector('.mode-tab.active')?.dataset.mode||'Classic',id=Number((($('#puzzleTitle')?.textContent||'').match(/#(\d+)/)||[])[1]);return puzzles.find(p=>p.id===id&&p.mode===mode)||null}
