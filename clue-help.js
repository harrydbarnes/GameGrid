(() => {
  ...
  const articleRule='Leading articles such as “The”, “A” and “An” count as part of the official title.';
  const oneWordRule='The official title must contain exactly one word. Leading articles such as “The”, “A” and “An” count as words, so “The Sims” does not qualify because it contains two words.';
  ...
  function description(id,label){
    ...
    if(id==='pre1990')return 'The game must have first released before 1990. This is exclusive of 1990: 1989 qualifies; 1990 does not.';
    if(id==='pre2000')return 'The game must have first released before 2000. This is exclusive of 2000: 1999 qualifies; 2000 does not.';
    if(id==='post2015')return 'The game must have first released in 2015 or later. 2015 is included.';
    if(id==='y1990s')return `The game's first release year must be within this range, inclusive: ${yearsText(1990,1999)}.`;
    if(id==='y2000s')return `The game's first release year must be within this range, inclusive: ${yearsText(2000,2009)}.`;
    if(id==='y2010s')return `The game's first release year must be within this range, inclusive: ${yearsText(2010,2019)}.`;
    if(id==='y2020s')return `The game's first release year must be within this range, inclusive: ${yearsText(2020,2029)}.`;
    ...
    if(rangeLetters[id])return `The official title must begin with ${rangeLetters[id]}. ${articleRule} This means “The Last of Us” begins with T for this clue.`;
