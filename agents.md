...
- The browser search index is the curated union of games used by scheduled puzzle intersections, not the full raw source catalogue. `KNOWN_PUBLISHER_ALIASES` contains small, reviewed browser-only metadata corrections for omitted maker fields; add a regression test for each correction and do not change historical answer counts without an explicit reset request.
